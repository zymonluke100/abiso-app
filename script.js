...
function toggleLang() {
    ...
    updateHistory(); // Update summary text too
}
...
database.ref('sensor_data').on('value', (snapshot) => {
    ...
    // AIR PILL LOGIC (Fixed conditional)
...
// 5. HISTORY DROPDOWN LOGIC
function updateHistory() {
    const wMonth = document.getElementById('wMonth')?.value || 'jul';
    ...
    document.getElementById('w-summary').innerText = isTl ? 
        `Ulat: Ang tubig para sa buwan ng ${wMonth.toUpperCase()} ay nanatiling ligtas.` : 
        `Summary: Water levels for ${wMonth.toUpperCase()} stayed within safe limits.`;
}
